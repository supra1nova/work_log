...
<div class="container">
  <div>Bbs list page</div>

  <form method="get" action="/bbs/list">
    <table class="mb5">
      <colgroup>
  ...
  <table class="mb5 hover-table table-layout-fixed">
    <colgroup>
      ...
      <col style="min-width: 275px; width: 275px">
    ...
    <c:forEach items="${list}" var="item">
      <c:if test="${item.active.equals('Y')}">
        <tr>
          <td class="tc">
            <a href="/ajax-file-bbs/view/${item.bbsSeq}">${item.bbsGrp}</a>
          </td>
          <td class="ellipsis-no-wrap">
            <a href="/ajax-file-bbs/view/${item.bbsSeq}"><c:forEach begin="0" end="${item.bbsDep}">&nbsp;</c:forEach>${item.bbsTitle}</a>
          </td>
          <td class="tc"><a href="/ajax-file-bbs/view/${item.bbsSeq}">${item.bbsRegId}</a></td>
          <td class="tc"><a href="/ajax-file-bbs/view/${item.bbsSeq}">${item.bbsRegDatetime}</a></td>
          <td class="tc">
            <a href="/ajax-file-bbs/view/${item.bbsSeq}">
              <c:if test="${!empty item.bbsUpdDatetime}">${item.bbsUpdDatetime}</c:if>
              <c:if test="${empty item.bbsUpdDatetime}">-</c:if>
            </a>
          </td>
          <td class="tc">
            <c:set var="savedFileListArr" value="${fn:split(item.savedFileList, ',')}" />
              ${!empty item.savedFileList && fn:length(savedFileListArr) > 0 ? fn:length(savedFileListArr) : '-'}
          </td>
        </tr>
      </c:if>
      <c:if test="${item.active.equals('N')}">
        <tr>
          <td colspan="6" class="tc">삭제된 글입니다</td>
        </tr>
      </c:if>
    </c:forEach>
    <c:if test="${empty list}">
      <tr><td colspan="6" style="text-align: center">게시글이 존재하지 않습니다</td></tr>
    </c:if>
    </tbody>
  </table>
  ...
  <div class="mb150 tr">
    <button type="button" onclick="location.href='/bbs/add'">새 글 작성</button>
  </div>
